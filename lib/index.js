...
function patchHostInventory(loader) {
  ...
  for (const file of candidates) {
    try {
      let content = readFileSync(file, 'utf8')
      if (content.includes('function packageDescription') && content.includes('description: packageDescription')) continue
      ...
      if (!content.includes('function packageDescription')) {
        const descFn = `
...
function packageDescription(moduleName) {
  ...
  const roots = [
    join(process.env.DSH_HOME || process.env.USERPROFILE || ".", "node_modules"),
    join(process.env.DSH_HOME || process.env.USERPROFILE || ".", ".dsh", "profiles", "node_modules"),
  ];
  try {
...
function patchAgentPresetClient(loader) {
  ...
  for (const file of candidates) {
    try {
      let content = readFileSync(file, 'utf8')
      if (content.includes('preset-switch-dropdown-v6')) continue

      const newFunction = `// preset-switch-dropdown-v6
