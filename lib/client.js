...
.pt-item .name{font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.pt-item .id{color:var(--theme-text-secondary,#888);font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:28%}
.pt-item .desc{color:var(--theme-text-secondary,#888);font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:34%}
            ...
            : React.createElement(
                ...
                entries.map((entry) =>
                  React.createElement(
                    ...
                    React.createElement("span", { className: "name", title: entry.description || entry.name }, entry.name),
                    React.createElement("span", { className: "id", title: entry.leafId || entry.id }, entry.leafId || entry.id),
                    React.createElement("span", { className: "desc", title: entry.description || "" }, entry.description || ""),
                    React.createElement(
                      "span",
    ...
    function apply(ctx) {
      ...
      if (slots) {
        ctx.effect(() => slots.inject("settings.section", () => slots.register({
          ...
          order: 60,
          label: () => "热插拔",
        }, PluginToggleSection)), "dsh-plugin-toggle: settings page");
      }
    }
